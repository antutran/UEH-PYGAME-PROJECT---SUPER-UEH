...
class Deadline(Enemy):
    ...
    def constraint(self):
    # Check if the rect is within the bounds of main_rect
        if not self.main_rect.contains(self.rect):
            # Correct position when it slightly lags at the edges
            if self.rect.right > self.main_rect.right:
                self.rect.right = self.main_rect.right - 10  # Snap to the right edge with a slight offset
            elif self.rect.left < self.main_rect.left:
                self.rect.left = self.main_rect.left + 10  # Snap to the left edge with a slight offset

            # Reverse direction and flip the frames
            self.direction *= -1
